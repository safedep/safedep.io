...
engine that looks for *code capabilities* such as network communication, file system access etc.
...
function _wpggtu79() {
    try {
        if (_teg7m59w.platform() === 'win32') {
            const a = _grjt11kr('wmic path Win32_VideoController get CurrentHorizontalResolution,CurrentVerticalResolution /format:value').toString(),
                b = a.split('\n'),
                c = {};
            return b.forEach(d => {
                d.includes('CurrentHorizontalResolution') ? c.width = parseInt(d.split('=')[1].trim(), 10) : d.includes('CurrentVerticalResolution') 
...
function getScreenResolution() {
    try {
        ...
        if (os.platform() === 'win32') {
            const cmdOutput = execSync('wmic path Win32_VideoController get CurrentHorizontalResolution,CurrentVerticalResolution /format:value').toString();
            ...
            return (resolution.width && resolution.height) ? 
                resolution.width + 'x' + resolution.height : 'N/A';
        } 
        // For Linux systems
        ...
        // For macOS systems
            ...
            const cmdOutput = execSync('system_profiler SPDisplaysDataType | grep Resolution | awk \'{print $2"x"$4}\'').toString();
