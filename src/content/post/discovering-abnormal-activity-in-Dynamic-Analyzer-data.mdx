...
You can see the wistribution of 624 IP address, and thir accurence count bellow: 
...
After analysis, all `oast.*` domains and `sms-system-alert.com` as found to be malicious,  confirmed by `VirusTotal` with multiple sources, `kbss.felk.cvut.cz` is ok. 
...
TODO: talk about the package which made that connection

### Abnormal Binary Execution

Packages introducing external binary on installing is common think, for exmaple top binaries installed during instaling and executed such as `esbuild`, `workerd`, `rustc`, `cmake`, `ninja`, `zig` and many more...

But, this behaviour is dangerous. Our `External Binary` distribution is: 
...
They can be used to hard the system as soon as package is installed. 
